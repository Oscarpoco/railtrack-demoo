...
export function PassengerLines({ vehicles }) {
  const trains = vehicles.filter((v) => v.vehicleType === "train");
  const railRoutes = Object.values(routes).filter((r) => r.vehicleType === "train");
    ...
    <div className="passenger-stack">
      <motion.section className="panel" variants={fadeUp} initial="hidden" animate="show">
        <div className="panel-hdr">
          ...
          <span className="cnt-tag">{railRoutes.length} lines</span>
        </div>

        <div className="route-summary">
          <div className="route-grid">
            {railRoutes.map((r) => {
              const rv = trains.filter((v) => v.routeId === r.id);
                ...
                >
                  <div className="rc-top">
                    <span className="rc-badge" style={{ background: r.color }}>
                      <Icon name="train" size={10} style={{ marginRight: 3 }} />
                      {r.shortName}
                    </span>
