/**
 * ControlCenter.jsx — Operations control centre, grouped by route, Framer Motion
 */
...
export function ControlCenter({ vehicles, alerts, onDismiss, onSimulateDelay, onClearDelay, onSendAlert }) {
  ...
  const rGroups = Object.values(routes).map(r => ({
    route: r, vehicles: vehicles.filter(v => v.routeId === r.id)
  }));
  ...
  function sendMsg(id) {
    ...
    if(m){ onSendAlert(id,m); setAlertMsgs(p=>({...p,[id]:""})); }
  }
    ...
    <motion.div className="page cc-page" variants={stagger} initial="hidden" animate="show">
      ...
      <motion.div className="occ-banner" variants={fadeUp}>
        ...
        <div className="occ-right">
          <div className="live-tag"><span className="live-dot"/>Simulation Active</div>
          <div className="occ-clock">
            {now.toLocaleString("en-ZA",{weekday:"short",year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"})}
          </div>
        </div>
      </motion.div>

      <div className="cc-grid">
        {/* Left — route control blocks */}
        <motion.div className="cc-left" variants={stagger}>
          {rGroups.map(({ route:r, vehicles:rv }) => {
            const isBus = r.vehicleType === "bus";
            return (
              <motion.section key={r.id} className="panel" variants={fadeUp}>
                <div className="panel-hdr" style={{ borderLeft:`4px solid ${r.color}` }}>
                  ...
                  <span className="cnt-tag">{rv.length} vehicle{rv.length!==1?"s":""}</span>
                </div>

                <table className="cc-table">
                  <thead>
                    <tr>
                      <th>Vehicle ID</th><th>Status</th><th>Direction</th>
                      <th>Next Stop</th><th>ETA</th><th>Speed</th><th>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rv.map(v => {
                      ...
                      return (
                        <>
                          <motion.tr key={v.id}
                            ...
                            <td><span className="vid" style={{color:r.color}}>{v.id}</span></td>
                            <td>
                              <motion.span
                            ...
                            <td>
                              <div className="act-row">
                                {isDelayed
                                  ? <motion.button className="btn btn-ok" whileTap={{scale:0.94}}
                                      onClick={e=>{e.stopPropagation();onClearDelay(v.id);}}>
                                      <Icon name="check" size={12}/>Clear
                                    </motion.button>
                                  : <motion.button className="btn btn-warn" whileTap={{scale:0.94}}
                                      onClick={e=>{e.stopPropagation();applyDelay(v.id);}}>
                                      <Icon name="delay" size={12}/>Delay
                                    </motion.button>
                                }
                                <motion.button className="btn btn-ghost" whileTap={{scale:0.94}}
                            ...
                            {isExp && (
                              <tr key={`${v.id}-exp`}>
                                <td colSpan={7} style={{padding:0}}>
                                  <motion.div className="exp-row"
                                    initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} exit={{opacity:0,height:0}}
                                    transition={{duration:0.25}}>
                                    <div className="exp-sec">
                                      <label className="exp-lbl"><Icon name="delay" size={11}/>Delay (minutes)</label>
                                      <div className="exp-inp-row">
                                        <input className="cc-input" type="number" min="1" max="60"
                                          value={delayMins[v.id]??10}
                                          onChange={e=>setDelayMins(p=>({...p,[v.id]:e.target.value}))}
                                          onClick={e=>e.stopPropagation()}/>
                                        <motion.button className="btn btn-warn" whileTap={{scale:0.94}}
                                          onClick={e=>{e.stopPropagation();applyDelay(v.id);}}>Apply</motion.button>
                                      </div>
                                    </div>
                                    <div className="exp-sec exp-wide">
                                      <label className="exp-lbl"><Icon name="broadcast" size={11}/>Broadcast Alert</label>
                                      <div className="exp-inp-row">
                                        <input className="cc-input cc-wide" type="text"
                                          placeholder="Enter message…"
                                          value={alertMsgs[v.id]??""}
                                          onChange={e=>setAlertMsgs(p=>({...p,[v.id]:e.target.value}))}
                                          onClick={e=>e.stopPropagation()}
                                          onKeyDown={e=>{ if(e.key==="Enter") sendMsg(v.id); }}/>
                                        <motion.button className="btn btn-blue" whileTap={{scale:0.94}}
                                          onClick={e=>{e.stopPropagation();sendMsg(v.id);}}>
                                          <Icon name="broadcast" size={12}/>Send</motion.button>
                                      </div>
                                    </div>
                                    <div className="exp-sec">
                                      <label className="exp-lbl"><Icon name="person" size={11}/>
                                        Passenger Load — {Math.round(v.passengerLoad/v.capacity*100)}%
                                      </label>
                                      <div className="exp-load-track">
                                        <motion.div
                                          className={`load-fill ${v.passengerLoad/v.capacity>=1?"lf-full":v.passengerLoad/v.capacity>=0.8?"lf-hi":"lf-ok"}`}
                                          initial={{width:0}}
        ...
        {/* Right — live map + mini cards */}
        <motion.div className="cc-right" variants={fadeUp}>
          <section className="panel">
            <div className="panel-hdr">
              <div className="panel-ttl"><Icon name="signal" size={14}/>Live Position Feed</div>
              <span className="live-tag"><span className="live-dot"/>LIVE</span>
            </div>
            <div className="map-wrap map-wrap-cc"><MapView vehicles={vehicles}/></div>
          </section>

          <section className="panel">
            <div className="panel-hdr">
              <div className="panel-ttl"><Icon name="dashboard" size={14}/>System Overview</div>
            </div>
            <div className="mini-grid">
              {vehicles.map(v => {
                ...
                return (
                  <motion.div key={v.id} className={`mini-card ${v.status==="Delayed"?"mc-del":""}`}
                    whileHover={{y:-3,boxShadow:`0 8px 20px ${r.color}22`}}
                    transition={{type:"spring",stiffness:300}}>
                    <div className="mc-id" style={{color:r.color}}>{v.id}</div>
                    <div className="mc-name">{v.name}</div>
                    <div className="mc-row"><span>Line</span><span>{r.shortName}</span></div>
                    <div className="mc-row"><span>Load</span><span>{lp}%</span></div>
                    <div className="mc-bar">
                      <motion.div
                        className={`load-fill ${lp>=100?"lf-full":lp>=80?"lf-hi":"lf-ok"}`}
                        initial={{width:0}} animate={{width:`${Math.min(100,lp)}%`}}
                        transition={{duration:0.6}}/>
                    </div>
                  </motion.div>
                );
